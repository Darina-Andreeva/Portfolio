...
class dashboard_sticky_notes {
    ...
    function dashboard_sticky_notes_wp_dashboard_setup() {
        global $current_user;
        $posts = get_posts(array('post_type' => 'dsn', 'posts_per_page' => -1));
    ...
    function dashboard_sticky_notes_add_meta_box() {
        ...
        $dsn_target = get_post_meta($post->ID, 'dsn_target', true);
        if (empty($dsn_target))
            $dsn_target = array('administrator');
        ?>
    ...
    function dashboard_sticky_notes_save_post() {
        ...
        if (!isset($post->post_type) || $post->post_type != 'dsn')
            return;
        ...
        if (!empty($dsn_context))
            update_post_meta($post_id, 'dsn_context', $dsn_context);
        if (!empty($dsn_priority))
            update_post_meta($post_id, 'dsn_priority', $dsn_priority);
        if (!empty($dsn_target))
